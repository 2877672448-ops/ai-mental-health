...
import { ChatRound, DeleteFilled, Clock, Plus, Promotion } from '@element-plus/icons-vue'
...
const loadSessionEmotion = (sessionId) => {
   // 确保sessionID格式正确
    ...
    getSessionEmotion(id).then(res => {
        ...
        // 情绪分析失败也不阻断页面
        console.warn('情绪分析加载失败：', err?.message || err)
    })
}
...
const getSessionPage = () => {
    ...
    getSessionList({
        ...
        // 只吞掉错误，不阻断页面；错误提示已由 request.js 全局处理
        console.warn('会话列表加载失败，继续显示页面：', err?.message || err)
    })
}
...
const handleSessionClick = (session) => {
    ...
    getSessionDetail(session.id).then(res => {
        ...
        console.warn('获取会话详情失败：', err?.message || err)
