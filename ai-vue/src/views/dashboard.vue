...
const initEmotionChart = () => {
  ...
  if (TrendData.length === 0) {
    emotionChart.setOption({ title: { text: '暂无数据', left: 'center', top: 'center' } })
    return
  }
  // 配置项
  const option = {
...
const initConsultationChart = () => {
  ...
  if (dailyTrend.length === 0) {
    consultationChart.setOption({ title: { text: '暂无数据', left: 'center', top: 'center' } })
    return
  }
  const option = {
    title: {
...
const initUserActivityChart = () => {
  ...
  if (activityData.length === 0) {
    userActivityChart.setOption({ title: { text: '暂无数据', left: 'center', top: 'center' } })
    return
  }
  const option = {
    title: {
