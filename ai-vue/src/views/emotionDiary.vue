...
<script setup>
    ...
    const submit = async () => {
        console.log('Submitting diary:', diaryForm)
        if (!diaryForm.moodScore) {
            ElMessage.error('请选择情绪评分')
            return
        }
        try {
            // 构造符合后端 EmotionDiary 实体的数据
            const payload = {
                mood_score: diaryForm.moodScore,
                dominant_emotion: diaryForm.dominantEmotion,
                emotion_triggers: diaryForm.emotionTriggers,
                diary_content: diaryForm.diaryContent,
                sleep_quality: diaryForm.sleepQuality,
                stress_level: diaryForm.stressLevel
            }
            await addEmotionDiary(payload)
